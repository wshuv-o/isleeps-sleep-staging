...
reports only sleep stages. A clinical polysomnogram records far more, and here the disease
...
multi-task model reading fourteen neural and cardiorespiratory channels, which produces two
outputs from one
...
the respiratory decision. On iSLEEPS, a stroke-specific
...
with $2.76$\,M trainable parameters. Against matched single-task models, identical but for
the loss, the joint objective leads on both tasks, by $0.006$ staging accuracy in nine
folds of ten and $0.007$ respiratory AUC in seven. We benchmark thirteen deep architectures retrained
