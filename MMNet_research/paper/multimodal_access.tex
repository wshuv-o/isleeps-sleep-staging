...
$\approx$ marks an accuracy inferred from a paper's macro-F1, $^{\S}$ a published
figure measured on SHHS rather than Sleep-EDF.}
\label{tab:reimpl}
\centering
...
is large and it is \emph{shared}, because the difficulty lies in the patients and not the
...
burden and not an index, and why event-level scoring is the natural next step instead of a
formality.
...
The association is real but modest, and it is worth saying why, not only that it is
...
establishes is that the \emph{ordering} is real and not sampling noise. The ordering is the
...
The two experiments answer different questions and the divergence is informative and not
