...
pointing at cohort difficulty and not an implementation artefact. Repeating the architecture
...
than the brain alone. The answer this cohort gives is worth stating at the outset, because it
is not the expected one. Reading the cardiorespiratory channels does not make sleep staging
more accurate: the model stages at $0.741$ from the neural stream alone and $\mmAcc$ with
both, a difference well inside fold-to-fold noise. What those channels buy is a second
clinical output, respiratory-event detection at $\mmAuc$ AUC, obtained from the same forward
pass at no cost to the first. The two physiologies carry complementary information about
different questions instead of redundant information about one, and the experiments below are
arranged to test exactly that.
...
reflects the difficulty of this cohort and not a weakness of our implementations. The five
...
representation and not to this encoder in particular: random encoders of comparable capacity
...
reaches $0.774$ AUC, at or just below the zeroed arm's $0.776$, so a misaligned
cardiorespiratory signal is worth no more than none at all. The bypass is a supporting design
choice on this evidence rather than a load-bearing one, and we treat it as such.
...
cost worth quantifying: the reported configuration was chosen by comparing forty-nine
...
and not the particular encoder that implements it. The accuracy is unaffected either way,
...
This is the result the paper is organised around, so it should be read for what it is rather
than as a disappointment. Breathing is not what sleep stages are scored from, and a
cardiorespiratory stream that improved staging would be the surprising outcome, not this one.
What matters clinically is the pair of numbers together: the second modality delivers
respiratory-event detection at $\mmAuc$ AUC while leaving staging exactly where it was. A
model that bought the second output by giving up accuracy on the first would be a poor
trade for a reader who needs both, and the ablation grid of Section~\ref{sec:ablate} shows
the same separation from the other side, each stream serving the output its physiology is
closest to. That the small staging gain
