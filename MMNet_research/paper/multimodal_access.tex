...
and report both heads at every point, because the failure worth finding is not a lower number
on one head but a trade between them (Table~\ref{tab:losssens}).
...
\begin{table}[t]
\caption{Sensitivity to the two loss constants, ten-fold patient-independent, three seeds
per value. Everything else is held at the published configuration; $^{\star}$ marks the
published value, which is read from the headline run rather than re-trained.}
\label{tab:losssens}
\centering
\small
...
\begin{tabular}{lcc}
\toprule
Value & Staging acc\,$\uparrow$ & Resp.\ AUC\,$\uparrow$ \\
\midrule
\multicolumn{3}{@{}l}{\emph{Task balance $\lambda$}}\\[1pt]
$0.25$ & $0.7370$ & $0.7660$ \\
$0.5$  & $0.7391$ & $0.7772$ \\
$1^{\star}$ & $\mmAcc$ & $\mmAuc$ \\
$2$    & $\mathbf{0.7403}$ & $\mathbf{0.7840}$ \\
$4$    & $0.7373$ & $0.7795$ \\
\addlinespace[2pt]\midrule
\multicolumn{3}{@{}l}{\emph{Respiratory positive weight $\pi$}}\\[1pt]
$1$    & $0.7399$ & $0.7778$ \\
$2.5$  & $0.7388$ & $0.7779$ \\
$5.35^{\star}$ & $\mmAcc$ & $\mmAuc$ \\
$8$    & $0.7401$ & $\mathbf{0.7846}$ \\
$11$   & $\mathbf{0.7422}$ & $0.7828$ \\
\bottomrule
\end{tabular}
\end{table}
