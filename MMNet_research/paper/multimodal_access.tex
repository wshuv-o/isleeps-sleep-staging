...
encoder. Both encoders are drawn with their internals expanded as isometric layer blocks, since
each is instantiated once. Block height encodes units for the multilayer perceptron and
temporal length for the convolutional stack, whose depth encodes channels, so the trade
that stack makes is visible: time resolution is spent to buy channels,
$750\rightarrow23$ samples against $7\rightarrow96$ channels. The two embeddings are fused and read across the night by a bidirectional LSTM. A
