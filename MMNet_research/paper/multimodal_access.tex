...
\caption{\stale{[STALE -- confusion matrix not yet regenerated for the final model]} Row-normalised confusion matrix for the proposed model (pooled ten-fold test epochs,
hidden-Markov operating point). Errors concentrate on N1, which is absorbed into Wake and N2.}
\label{fig:confusion}
\end{figure}

\begin{table}[t]
\caption{\stale{[STALE -- per-class F1 not yet regenerated for the final model]} Per-class staging F1 (Wake, N1, N2, N3, REM), ten-fold patient-independent, at the
...
\caption{\stale{[STALE -- modality ablation grid not yet regenerated for the final model]} Modality-ablation grid. Staging accuracy (blue) and respiratory AUC (red) as each
...
\caption{\stale{[STALE -- operating characteristics not yet regenerated for the final model]} Respiratory-event detection, operating characteristics, pooled over the ten
...
\begin{table}[t]
\caption{\stale{[STALE -- respiratory detection table not yet regenerated for the final model]} Respiratory-event detection, ten-fold patient-independent. \textit{(a)} Detectors trained
...
\caption{\stale{[STALE -- AHI scatter not yet regenerated for the final model]} Predicted per-patient event burden (mean respiratory probability) against the clinical
...
\begin{table}[t]
\caption{\stale{[STALE -- severity table not yet regenerated for the final model]} Sleep-staging accuracy by sleep-disordered-breathing severity (AASM AHI bands),
...
\caption{\stale{[STALE -- learning curve not yet regenerated for the final model]} Learning curve over training-patient fraction, ten-fold patient-independent, error
...
\begin{table}[t]
\caption{\stale{[STALE -- external validation not yet regenerated for the final model]} External validation on two public corpora. One model trained on all $96$ iSLEEPS
...
respiratory head cannot be scored there.}
\label{tab:ext}
\centering
\small
...
\midrule
iSLEEPS (in-domain)   & stroke        & 96 & $\mmAcc$ & $\mmK$ & $\mmAuc$ \\
\midrule
Sleep-EDF             & healthy       & 11 & $\mathbf{0.800}$ & $\mathbf{0.712}$ & --- \\
