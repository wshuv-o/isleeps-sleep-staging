...
\label{app:features}
Each $30$\,s epoch is $3000$ samples at $100$\,Hz. Every descriptor below is
computed on one epoch in isolation, with no smoothing between epochs; cross-epoch
context comes from the recurrent decoder instead. The vector is $23$ descriptors on
each of the seven neural channels (C4:M1, C3:M2, O2:M1, O1:M2, E1:M2, E2:M2, chin
EMG), which is $161$, and $27$ event descriptors applied only to the channels they
are defined for, giving $188$.

\subsubsection*{Per-channel descriptors}
Spectra are Welch estimates, $256$-sample segments with $128$-sample overlap. Let
$P(f)$ be the spectrum and $T=\sum_f P(f)$.

\begin{itemize}\itemsep2pt
\item Absolute and relative power in five bands: delta $0.5$--$4$, theta $4$--$8$,
alpha $8$--$13$, sigma $11$--$16$, beta $16$--$30$\,Hz. Sigma is the spindle band
and overlaps alpha by design.
\item $\log T$, spectral entropy $-\sum_f \tilde P\log \tilde P$ on
$\tilde P=P/T$, the $95\%$ spectral edge frequency, and the mean frequency
$\sum_f fP(f)/T$.
\item Standard deviation, peak-to-peak, root mean square, skewness, kurtosis, and
zero-crossing rate.
\item Hjorth activity $\sigma^2_x$, mobility $\sigma_{x'}/\sigma_x$, and complexity
$(\sigma_{x''}/\sigma_{x'})/\text{mobility}$, from the first and second
differences~\cite{hjorth1970eeg}.
\end{itemize}

\subsubsection*{Event descriptors}
These follow the graphoelements a scorer reads. Band-pass filters are fourth-order
Butterworth applied forwards and backwards, so phase is preserved.

\begin{itemize}\itemsep2pt
\item \emph{Spindles}, three per EEG channel. The channel is filtered to
$11$--$16$\,Hz and the analytic envelope taken by Hilbert transform. The threshold
is the $90$th percentile of that envelope within the epoch, so it tracks the
epoch's own amplitude instead of a fixed microvolt level. Density counts upward
crossings; the remaining two are the envelope mean and standard deviation.
\item \emph{Slow waves}, two per EEG channel. Filtered to $0.5$--$4$\,Hz, then
peak-to-peak and mean absolute amplitude. No duration or morphology criterion is
applied, so these summarise slow-wave amplitude and are not discrete detections.
\item \emph{Ocular}, two per EOG channel: the log mean squared first difference and
the $95$th percentile of its absolute value, which separate REM deflections from
slow drift.
\item \emph{Chin EMG}, three: log root-mean-square, the $90$th percentile of the
absolute signal, and the log variance of the first difference, tracking tonic level
and its loss in REM.
\end{itemize}

\subsubsection*{Artifacts, scaling and aggregation}
No epoch or channel is rejected and no amplitude threshold is applied, so a
corrupted channel reaches the model. The modality ablations in
Section~\ref{sec:ablate} show the streams carry enough redundancy to absorb one,
and any rejection rule would have to be tuned on this cohort. Non-finite values,
which occur only for a flat channel, become zero. Descriptors are standardised per
recording against that recording's own mean and standard deviation, the
transductive step Section~\ref{sec:limitations} returns to. Nothing is aggregated
across epochs.
