...
against $0.140$ to $0.148$ for those baselines (Section~\ref{sec:domaingap}). A better model
may yet do better; what these results rule out is that the implementations are what hold the
numbers down.
...
subacute-stroke cohort, show that the healthy-built networks degrade to near-chance accuracy, and establish by reimplementation that this degradation is a property of the
cohort.
...
is worth. Section~\ref{sec:repr} quantifies the effect of this substitution, which is the
largest of any representation change we measure, and
Sections~\ref{sec:repr} and~\ref{sec:nested} together show that it belongs to the change of
...
cohort size. We therefore claim the change of representation, as Section~\ref{sec:repr} says,
itself. The encoder implementing it is interchangeable at this cohort size, and the
