...
is worth. The effect is the largest of any representation change we measure, and
...
over the same raw signal recover much of it. We use this one because it is small, trained
end-to-end with the rest of the network, and adds no external dependency, and we claim the
representation.
...
same $2{,}764{,}774$ parameters. A third arm tests the sharper version of the question by
...
selection with the test patients quarantined (Section~\ref{sec:nested}): five outer
...
experiment, and treating them as thirty would triple
...
The headline figure survives the quarantine. The nested procedure reaches
$0.7336 \pm 0.0089$ accuracy across the five outer folds against $\mmAcc$ for the reported
model, so selection is worth $0.006$ accuracy, about a third of the fold-to-fold standard
deviation and smaller than any difference this paper rests an argument on. It is also close
to the $0.004$ suggested by the spread of the top ten candidates, so the bound available
without a nested loop was about the right size.
...
encoder, the BiLSTM, and the same width and regularisation, which is a five-fold independent
vote for the neural half of the architecture. Where the folds differed from the published
...
cleanly, and we mark it as such.
...
These results identify the representation of the cardiorespiratory stream, rather than the detector above it, as the factor limiting the respiratory read-out; the detector is held fixed throughout. We therefore claim the
...
questions at once, in a population in which both are clinically relevant. The learned
...
on. Whole-night decoding therefore remains the primary constraint on real-time bedside
