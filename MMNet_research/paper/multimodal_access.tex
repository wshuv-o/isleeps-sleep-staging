...
$2.76$\,M trainable parameters. We benchmark nine deep architectures retrained on this
cohort and three applied zero-shot from healthy sleep, all on identical folds; every
retrained baseline lands between $0.613$ and $0.686$ accuracy. Three are reimplemented
here, each recovering its published Sleep-EDF accuracy to within $0.03$, which establishes
that the ceiling belongs to the cohort and not the implementations. Cross-validated on
...
for staging (Section~\ref{sec:method}). The ablation identifies respiratory effort, not
oxygen saturation, as the channel that read-out depends on (Section~\ref{sec:ablate}).

\item \label{c:bench}We benchmark the model against twelve baseline configurations on identical
...
accuracy to within $0.03$, every retrained baseline lands between $0.613$ and $0.686$
...
only respiratory detection (Wilcoxon $p=0.004$), while removing the ocular channel lowers
...
models~\cite{velickovic2018gat} on a subacute-stroke cohort, show that the healthy-built
networks collapse, and establish by reimplementation that the collapse is a property of the
cohort.
...
the convolutional stack. Dashed outlines mark frozen parameters. Cross-modal fusion is
detailed at lower right.}
\label{fig:arch}
\end{figure*}
...
the branch compresses a $7\times750$ tensor to $64$ dimensions, and
Section~\ref{sec:repr} shows that widening it past $64$ buys nothing.
...
event. We report the connection as a design rationale and not as a measured gain: the
obvious test, removing it while holding everything else fixed, is not reported here, so a
reader should treat it as an architectural choice we have argued for but not yet isolated.
%
% INSERTION POINT for the bypass ablation now running (run_bypass_ablation.py,
% 3 seeds x 10 folds, with and without the direct path, Wilcoxon on 30 paired
% fold-values). Replace the two sentences above with the measured effect when the
% numbers land; if the effect is null, the design rationale comes out entirely.
% The trained model holds $2.76$ million parameters. The pretrained encoder that
supplies the $200$-dimensional neural embedding is run once offline and is not trained
here, so it adds no trainable parameters to this count.
...
$20$-subject Sleep-EDF Cassette subset ($40$ recordings, $21$ subjects), single
Fpz-Cz derivation. $\approx$ marks an accuracy inferred from the macro-F1 its paper
reports.}
\label{tab:reimpl}
\centering
...
(Table~\ref{tab:reimpl}). ``feat'' marks engineered features. Cells are mean $\pm$ SD
across folds where per-fold values were retained. Best per column in \textbf{bold}.}
\label{tab:bench}
\centering
...
\multirow{10}{*}{\shortstack[l]{\emph{Deep networks,}\\\emph{retrained on iSLEEPS}}}
 & LSTM~\cite{maiti2026isleeps}                  & 1\,EEG  & n/r          & --- & \textbf{0.747} & --- & --- \\
...
\multirow{3}{*}{\shortstack[l]{\emph{Trained on healthy sleep,}\\\emph{applied zero-shot}}}
 & CNN                                           & 4\,EEG  & $0.49$\,M    & 0.806 & 0.325 & 0.237 & 0.111 \\
 & CNN\,+\,BiLSTM (CRNN)                         & 4\,EEG  & $1.15$\,M    & 0.834 & 0.307 & 0.251 & 0.111 \\
 & StagingSeqNet                                 & 4\,EEG  & $1.08$\,M    & \textbf{0.868} & 0.308 & 0.232 & 0.093 \\
\midrule
\multirow{2}{*}{\shortstack[l]{\emph{Proposed feature-based}\\\emph{multi-task model}}}
 ...
 & \textbf{MM-Net (neural + cardio)} & 14\,ch feat & 2.76\,M & --- & $0.739 \pm 0.020$ & $\mathbf{0.670 \pm 0.020}$ & $0.634 \pm 0.024$ \\
\midrule
\emph{Same model, healthy sleep}
 & MM-Net (neural only)              & 2\,EEG feat & 2.62\,M & $0.856 \pm 0.027$ & --- & --- & --- \\
\bottomrule
\end{tabular*}
\end{table*}
...
Dispersion is reported wherever per-fold values survive: five folds for the earlier retrained baselines, ten for the three reimplemented
literature models, and thirty fold-values (ten folds, three seeds) for the proposed rows.
CNN-ResNet18 is quoted from the corpus paper and AttnSleep retained only aggregates, so
neither carries an interval, and the zero-shot block is a single pass over all $99$
patients with no fold structure. Sleep-EDF carries no cardiorespiratory channels, so every
...
healthy sleepers. The compact, feature-based model reaches $\mmAcc$, above every deep architecture we
...
$0.780$ AUC against $\mmAuc$ in-domain, a gap of $0.002$, and it does so handicapped,
because ISRUC provides no pulse channel and no separate effort trace, leaving two of the
seven cardiorespiratory inputs zero-filled. A detector that had latched onto one centre's
equipment would not survive that; this one does. Night two falls to $0.723$, tracking its
...
\textbf{MM-Net (proposed)}                & 14\,ch feat & \textbf{2.76\,M} & \textbf{stage + apnea} \\
\bottomrule
\end{tabular}
\end{table}
...
received the B.Sc. degree in computer science and engineering from American
International University-Bangladesh (AIUB), Dhaka, Bangladesh, in \stale{[year]}, where she
