...
reports only sleep stages. A clinical polysomnogram records far more: eye movements,
muscle tone, the electrocardiogram, airflow, breathing effort, and blood oxygen
saturation. In this population those channels carry the disease itself, and the median
patient in this cohort has a scored respiratory event in $13\%$ of epochs. We present
MM-Net, a two-stream multi-task model that reads the full polysomnogram and produces two
clinical outputs from a single forward pass: the sleep stage of every 30-second epoch and
a per-epoch respiratory-event label, with a direct cardiorespiratory path that
delivers breathing effort and desaturation to the respiratory decision undiminished by the
staging objective. On iSLEEPS, the first public
polysomnography corpus of subacute ischemic stroke ($99$ patients, $92{,}560$ epochs),
under patient-independent ten-fold cross-validation, MM-Net stages sleep at $\mmAcc$
accuracy (Cohen's $\kappa$ $\mmK$) and detects respiratory events at $\mmAuc$ AUC with
$2.76$\,M trainable parameters. We benchmark fourteen deep architectures retrained on this
cohort and three applied zero-shot from healthy sleep, all on identical folds; every
retrained baseline lands between $0.613$ and $0.716$ accuracy. Three are reimplemented
here, each recovering its published accuracy to within $0.04$, which indicates
that the ceiling belongs to the cohort and not to our implementations. Cross-validated on
healthy sleep the same architecture reaches $0.856$, losing $0.115$ accuracy crossing into
stroke against $0.140$ to $0.148$ for those baselines. Zero-shot on two independent public
corpora, the respiratory read-out holds at $0.780$ AUC against $\mmAuc$ in-domain, while
staging rises on healthy sleep and falls $0.090$ on a sleep-disorders cohort. Code, features and checkpoints are
released.
\end{abstract}
...
\item \label{c:bench}We benchmark the model against seventeen baseline configurations on identical
patient-independent folds, covering convolutional, recurrent, attentional and fully
...
leave-one-out modality-ablation grid shows removing the cardiorespiratory stream lowers
only respiratory detection ($-0.128$ AUC, Wilcoxon on thirty fold-values, Holm
$p<10^{-7}$), while removing the ocular channel lowers
only staging, giving a falsifiable account of what each modality contributes
...
engineered descriptors below with a $200$-dimensional embedding from a frozen pretrained EEG
encoder (Section~\ref{sec:repr}), and a raw cardiorespiratory tensor
...
concatenation used for every reported result; the panel at lower right details the attention
fusion evaluated against it in Section~\ref{sec:ablate}, which it matches to within noise.}
\label{fig:arch}
\end{figure*}
...
stream should be read as coming from the added modality and not from how it is combined.
...
signature lives in, and it is respiratory effort, not the oxygen fall, that names the
event. We isolate the connection by removing it and holding everything else fixed, on the
...
point. Every headline figure in this paper is the mean over all thirty fold-values, ten
folds by three seeds, and not a single run. The three seed means for respiratory AUC are
...
This is one subject more than the conventional Sleep-EDF-20 split, so the published
accuracies opposite are not computed on an identical set. $\approx$ marks an accuracy
inferred from the macro-F1 its paper reports; $^{\S}$ marks a model whose published figure
is on SHHS rather than Sleep-EDF, so its check compares different corpora and is weaker
than the others.}
\label{tab:reimpl}
\centering
...
SleepTransformer~\cite{sleeptransformer2022} & 0.848 & $0.823 \pm 0.033$ & $-0.025$ \\
1D-ResNet-SE-LSTM & 0.864 & $0.832 \pm 0.036$ & $-0.032$ \\
Micro SleepNet$^{\S}$ & 0.833 & $0.800 \pm 0.041$ & $-0.033$ \\
AT-BiLSTM & 0.838 & $0.835 \pm 0.046$ & $-0.003$ \\
\bottomrule
\end{tabular}
\end{table}
...
evaluated. $^{\dagger}$ marks a model reimplemented by us from its paper
(Table~\ref{tab:reimpl}). $^{\ddagger}$ marks a figure quoted from the corpus
paper~\cite{maiti2026isleeps} rather than run here: those two rows use that paper's
protocol, an $80/10/10$ patient-wise split over $N=100$ including the duplicated pair we
exclude (Section~\ref{sec:data}), and its own two-channel input; the authors' repository
releases preprocessing only, so neither row can be reproduced and neither is a
same-protocol comparison. Bold marks the best value among rows run under our protocol.
``feat'' marks engineered features. Cells are mean $\pm$ SD across folds where per-fold
values were retained.}
\label{tab:bench}
\centering
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}} l l l c c c c c @{}}
\toprule
 & & & & Healthy & \multicolumn{3}{c}{iSLEEPS (stroke)} \\
\cmidrule(lr){6-8}
 & Model & Input & Params\,$\downarrow$ & Acc & Acc\,$\uparrow$ & Macro-F1\,$\uparrow$ & $\kappa$\,$\uparrow$ \\
\midrule
\multirow{14}{*}{\shortstack[l]{\emph{Deep networks,}\\\emph{retrained on iSLEEPS}}}
 & LSTM~\cite{maiti2026isleeps}$^{\ddagger}$     & 1\,EEG\,$+$\,EOG & n/r & --- & $0.747$ & --- & --- \\
 & LSTM, our reimplementation$^{\dagger}$ & 1\,EEG\,$+$\,EOG & $1.16$\,M & --- & $0.720 \pm 0.032$ & $0.626 \pm 0.028$ & $0.599 \pm 0.043$ \\
 & CNN-ResNet18~\cite{maiti2026isleeps}$^{\ddagger}$ & 1\,EEG\,$+$\,EOG & $\sim$11\,M & --- & 0.617 & 0.544 & 0.480 \\
 ...
 & 1D-ResNet-SE-LSTM$^{\dagger}$ & 1\,EEG & $0.65$\,M & $0.832 \pm 0.036$ & $0.708 \pm 0.038$ & $0.631 \pm 0.033$ & $0.592 \pm 0.050$ \\
 & Micro SleepNet$^{\dagger}$ & 1\,EEG & $0.03$\,M & $0.800 \pm 0.041$ & $0.647 \pm 0.049$ & $0.558 \pm 0.028$ & $0.510 \pm 0.055$ \\
 & AT-BiLSTM$^{\dagger}$ & 1\,EEG & $0.92$\,M & $0.835 \pm 0.046$ & $0.692 \pm 0.040$ & $0.612 \pm 0.030$ & $0.570 \pm 0.051$ \\
...
\midrule
\multirow{3}{*}{\shortstack[l]{\emph{Trained on healthy sleep,}\\\emph{applied zero-shot}}}
...
Each reimplementation is checked against its published Sleep-EDF accuracy in
Table~\ref{tab:reimpl}; those published figures follow each paper's own protocol, which
differs in subset and split, so they are consistency checks and not exact reproductions.
...
reimplemented literature models and the proposed rows. The LSTM and CNN-ResNet18 rows are
quoted from the corpus paper rather than re-run here, so their protocol is that paper's and
not ours. The LSTM additionally appears as our own reimplementation on the folds used
throughout, which reaches $0.720 \pm 0.032$ over thirty fold-values against the $0.747$ they
report; that shortfall of $0.027$ is within the range our other reimplementations show
against their published figures and neither carries an interval. In the zero-shot block the interval is across
...
rediscover what decades of sleep science already encode in the features. It does not lead the
corpus. The published single-channel LSTM reaches $0.747$, above our $\mmAcc$, and we list it
in Table~\ref{tab:bench} instead of leaving the best published number off the comparison. No
method we are aware of exceeds $0.75$ on this corpus. Section~\ref{sec:curve} shows why: the
...
\textbf{The margin over the retrained baselines is tested, not eyeballed.} The folds are fixed
...
is large and it is \emph{shared}, because the difficulty lies in the patients rather than the
architecture --- AttnSleep spans $0.627$ to $0.755$ across folds --- so reading two overlapping
...
channel and measuring the change. This is a falsifiable test of what each modality
contributes, and not a post-hoc visualization.
...
configuration was built around, is here the most expendable channel
...
ensemble is essential. That redundancy is clinically favourable, not a weakness, since
...
\caption{Modality-ablation grid. Drop in staging accuracy and in respiratory AUC
...
Scoring at the event level instead of the epoch level changes the picture, and not
...
burden and not an index, and why event-level scoring is the natural next step rather than a
formality.
...
which is a screening signal, not a scoring-grade detector, and we describe it as
...
visible, not hidden; at $0.005$ it is an eighth of one fold standard deviation.
...
Detectors trained on the same folds. The classical baselines read the $14$
cardiorespiratory descriptors; MM-Net reads the raw signal through its learned encoder.
Chance-level average precision equals the event prevalence, $0.157$. \textit{(b)} AUC by event type
over the $96$ patients with event-type annotation. Best per column in \textbf{bold}.}
\label{tab:resp}
\centering
\small
\begin{tabular}{lcc}
...
clinician's summary measure, not only an epoch-level score. Staging, in turn, degrades as
...
\textbf{Permutation separates them where retraining cannot.} Each fold records its own baseline
...
establishes is that the \emph{ordering} is real rather than sampling noise. The ordering is the
...
The two experiments answer different questions and the divergence is informative rather than
contradictory. Retraining without a channel measures whether it is \emph{irreplaceable}, and
...
respiratory cue. Unlike an embedding projection, that was a prediction that could have failed.
...
the full held-out fold (Fig.~\ref{fig:curve}). Patients are subsampled, not epochs,
...
($\mmAcc$ and $\mmAuc$); the shape, not the endpoint, is what it is here to show. \textbf{Staging is at or near
...
labels and the injured brain, not of the training-set size. The final model also exceeds
...
as published. The three architectures are ours, not published models: a four-channel CNN, the same
...
\textbf{The same argument run in reverse.} Every figure above concerns other
...
That places it among the healthy-sleep stagers in the middle block, not below them,
...
\textbf{The cohort does not cost every architecture equally.} Because the three reimplemented
...
degrades least. We report this as an observation and not a claim. Four paired differences
will not support a significance test, and the comparison is between systems and not a
controlled ablation, since the proposed rows read engineered features and a frozen embedding
...
\begin{table*}[t]
\caption{What each stream should be represented as. One factor varies per block with
everything else held fixed; ten-fold patient-independent, three seeds, thirty fold-values
per cell, mean $\pm$ SD. $\Delta$ and Holm $p$ are against the engineered-feature baseline
of each block, Wilcoxon paired on fold-values. ``adopted'' marks the choice carried into the
final model.}
\label{tab:repr}
\centering
...
\multirow{4}{*}{\emph{Neural}}
 ...
 & frozen pretrained encoder alone      & $0.681 \pm 0.033$ & $0.712 \pm 0.033$ & $-0.028$ & $<0.001$ \\
...
genuinely hard epochs and not merely hedging, and it concentrates where a scorer would
...
\textbf{The respiratory read-out transfers.} On ISRUC night one the model reaches
...
The comparison needs its estimators matched to mean anything, and we state them rather than
...
much lower event prevalence ($2.4\%$ against $10.1\%$) and not anything about the
model.
...
\textbf{A montage explanation the data now rule out.} An earlier version of this
...
attributing it confidently. We keep the failed prediction on the record because it was
falsifiable, it was tested on a corpus chosen for an unrelated reason, and it was wrong.
...
$99$ iSLEEPS patients with no dataset-specific tuning. In-domain figures are ten-fold
patient-independent means. Sleep-EDF carries no cardiorespiratory channels, so the
respiratory head is not scored there. The two ISRUC nights differ sharply in event
prevalence and are reported separately.}
\label{tab:ext}
\centering
...
\caption{Two-dimensional t-SNE of the model's per-epoch embeddings (pooled test
epochs). \textbf{Left:} by sleep stage; Wake, N2, N3 and REM form distinct territories
while N1 is diffuse. \textbf{Right:} by respiratory-event label; events spread across the
manifold, consistent with the modest detection AUC.}
\label{fig:tsne}
...
\textbf{The system is offline, and mildly transductive.} Two properties follow from
...
\textbf{A prediction of ours that failed.} The montage account of N3 transfer described in
...
screening-grade one, and it is scored at the epoch level, not per event. At
...
posteriors should be used for ranking and abstention, not read as probabilities. Staging sits at
...
SleepTransformer~\cite{sleeptransformer2022} & 2022 & Transformer encoder with epoch-level interpretability and uncertainty estimates. & $0.848$ (Sleep-EDF) & $0.683$ \\
1D-ResNet-SE-LSTM & 2023 & Residual network with squeeze-and-excitation blocks and an LSTM on raw EEG. & $0.864$ (Sleep-EDF) & $0.708$ \\
Micro SleepNet & 2023 & Lightweight convolutional model for real-time mobile staging. & $0.833$ (SHHS) & $0.647$ \\
AT-BiLSTM & 2024 & Attention-augmented bidirectional LSTM, single-channel EEG. & $0.838$ (Sleep-EDF) & $0.692$ \\
\midrule
...
Odin, Dhaka, where he builds LLM-based document intelligence and financial reporting
systems, and a Co-Founder of Byteverse Ltd. and of Prodigy AI, a non-profit research group
focused on brain--computer interfaces. He is the lead author of an article on
symmetry-aware equivariant quantum clustering in \emph{Physica Scripta} and a co-author of
an article on Lorentz-equivariant quantum graph neural networks in the \emph{IEEE
Transactions on Artificial Intelligence} and of a Springer book chapter on federated
learning in healthcare. His research interests include brain--computer interfaces, EEG
decoding, quantum machine learning, and applied large language model systems. He was a
recipient of the Rising Star Performer Award from Odin in 2025.
\end{IEEEbiography}
...
deep learning for autonomous vehicle perception. Her research interests include machine
learning for health informatics, physiological time-series analysis, quantum and federated
learning, and multimodal deep learning.
\end{IEEEbiography}
...
book chapter on federated learning in healthcare technology. His research interests include
deep learning, quantum machine learning, biomedical signal analysis, EEG-based
