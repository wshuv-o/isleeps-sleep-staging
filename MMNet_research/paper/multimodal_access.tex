...
event prevalence, $0.157$. \textit{(b)} The proposed model's AUC by event type -- the rarer, more
severe events are detected best. Best per column in \textbf{bold}.
\stale{[Panel (b) is from the previous configuration: the per-event-type labels were not
retained with the final model's predictions, so it has not been regenerated. Panel (a) is
current.]}}
\label{tab:resp}
\centering
\small
...
on transfer rather than clean estimates of it.
\stale{[The two ISRUC rows are from the previous configuration and have not been regenerated;
that corpus was not reachable from the training machine.]}}
\label{tab:ext}
\centering
\small
