...
five folds for the retrained baselines, thirty fold-values (ten folds, three seeds) for the
proposed rows. CNN-ResNet18 is quoted from the corpus paper and AttnSleep retained only
...
neural branch alone and the comparison row is MM-Net (neural only). ``feat'' marks
engineered features. Best per column in \textbf{bold}.}
\label{tab:bench}
\centering
...
\midrule
\multirow{6}{*}{\shortstack[l]{\emph{Deep networks,}\\\emph{retrained on iSLEEPS}}}
...
$99$ iSLEEPS patients zero-shot, with no adaptation of any kind --- the middle block of
...
on healthy sleep --- ten-fold, subject-independent over the $21$ Sleep-EDF subjects, the
neural branch alone since that corpus has no cardiorespiratory channels --- and it reaches
$0.856 \pm 0.027$ accuracy with $\kappa$ $0.800$ (final row of Table~\ref{tab:bench}).
That places it among the healthy-sleep stagers in the middle block rather than below them,
...
Folds are split by subject rather than by recording, because Sleep-EDF Cassette records
