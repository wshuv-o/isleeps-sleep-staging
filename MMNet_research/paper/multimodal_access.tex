...
respiratory-event label. On iSLEEPS, a stroke-specific
...
with $2.76$\,M parameters. Cardiorespiratory input does not raise staging accuracy; it adds
the second output at no cost to the first. Against matched single-task models, identical but for
...
between $0.613$ and $0.720$, and the five reimplementations publishing
an accuracy are recovered to within $0.04$,
pointing at the difficulty of the cohort. Repeating the architecture
search inside the training data costs $0.006$ accuracy. On healthy sleep
...
ask a question that conventional staging evaluation does not:
what does a model gain, and on which clinical task, from reading the whole recording. We
state the answer here, since it governs how the results should be
read. The cardiorespiratory channels do not improve sleep staging: the model reaches $0.741$
accuracy from the neural stream alone and $\mmAcc$ with both, a difference well inside
fold-to-fold variation. They provide a second clinical output, respiratory-event
detection at $\mmAuc$ AUC, from the same forward pass and without reducing staging accuracy.
The two physiologies therefore carry complementary information about different clinical
questions, and the experiments below are designed to test that separation.
...
$\mmAuc$ AUC, with a direct cardiorespiratory path to the respiratory head that avoids
attenuation of
task-specific information during fusion; removing that path costs $0.006$ respiratory AUC and leaves staging unchanged
...
obtained without degrading performance on the first (Section~\ref{sec:joint}). The ablation identifies respiratory effort, not oxygen
...
\item \label{c:ceiling}We give converging evidence that the accuracy every method reaches here
reflects the difficulty of this cohort. The five
reimplementations whose papers report an accuracy
...
against $0.140$ to $0.148$ for those baselines (Section~\ref{sec:domaingap}). These results
provide evidence that the observed performance is not primarily attributable to
...
Together, these results reframe the problem on this cohort: the useful gains arise from
integrating neural and cardiorespiratory physiology and attributing each output to the
modality that supports it, while deeper single-channel staging does not help here. The remainder of the paper develops the model
...
subacute-stroke cohort, show substantial degradation of architectures developed on
healthy-sleep cohorts, and provide evidence by reimplementation that this degradation is
associated with the cohort rather than with implementation differences.
...
effect of this substitution, which is the
largest of any representation change we measure, and
...
earlier retrained baselines were run at five, which Table~\ref{tab:bench} states per row. Because performance estimates may vary
...
the full $\mmAuc$, so the model does not appear to depend primarily on the signal used for
event annotation. Respiratory effort provides the strongest contribution, being the only
channel whose removal costs significantly,
together with oxygen desaturation, beat-to-beat cardiac variability, and the
...
table supports a comparison it could not before: how much each model loses in the crossing. TinySleepNet falls $0.148$ from healthy sleep to stroke
...
attributing it confidently. The prediction was therefore not supported by the final-model
analysis.
...
\section{Discussion}
The result is a single model that reads both the neural and the cardiorespiratory channels of
the polysomnogram and addresses two clinical
questions at once, in a population in which both are relevant. The learned
per-epoch embedding separates by sleep stage while the
respiratory structure remains diffuse, consistent with the higher staging accuracy and the
lower respiratory AUC.

 Staging accuracy remained below $0.75$ across the model families evaluated on this cohort, and the proposed model reaches $\mmAcc$ while
adding a respiratory capability the staging models lack. The respiratory
read-out does not depend on the labelling channel: performance is retained after removal of
the airflow channel, with the remaining attribution concentrated on respiratory effort,
desaturation and arousal, the same signs a physician reads at the
bedside.

The $\mmAcc$ staging figure should be judged on this cohort, where healthy-sleep accuracy does not transfer. Deep
architectures developed on healthy cohorts show substantial performance degradation here:
DeepSleepNet reaches $0.615$,
...
those models learned to read. Physiologically grounded modelling on engineered
features retains accuracy of $0.72$ to $0.75$, matching the published baseline on this corpus.
Taken together, these observations indicate that architectures developed on healthy sleep do
not transfer to this population, and that cohort-specific physiologically grounded multimodal
modelling is required.

The modality-ablation results demonstrate a task-specific dependence on neural and
cardiorespiratory information: the cortical channels support sleep staging and the
cardiorespiratory channels support respiratory-event detection. That the neural and multimodal variants stage alike is the expected
outcome, since the second modality adds a capability while leaving staging unchanged. This separation is favourable for
clinical use, since each output can be attributed to the physiology it derives from.

Modality-specific contributions do not imply that the tasks should be trained independently.
Section~\ref{sec:joint} trains each task alone and finds the joint model ahead of both,
slightly and consistently. This is the behaviour a shared temporal decoder would predict: the
streams carry different information, and the recurrent state that reads them benefits from
being trained on two objectives over the same ten-minute window.
...
Future evaluation should include event-level metrics, including event sensitivity and false
alarms per hour, which are more directly relevant to clinical scoring than the epoch-level
AUC reported here. A further direction is to evaluate whether the multimodal representation
is associated with stroke severity and recovery outcomes.
...
stages their sleep and detects their respiratory events. Deep networks developed on
healthy-sleep cohorts achieved $0.61$--$0.72$ accuracy on iSLEEPS, whereas MM-Net achieved
$\mmAcc$, the highest of the model families evaluated here. Its principal contribution is the
concurrent respiratory read-out at $\mmAuc$ AUC, which is retained after removal of the
airflow channel and therefore does not depend on the signal the events were scored from.
Reading the full recording allows a single model to report on breathing and on sleep in a
population in which sleep-disordered breathing is prevalent. Code and features are released.
...
deep learning for autonomous vehicle perception. Her research interests span brain signal analysis, transformer models for EEG, brain--computer and brain--machine interfaces, and applied machine learning for health.
\end{IEEEbiography}
...
book chapter on federated learning in healthcare technology. His research interests are in transformer models for EEG, brain--computer and brain--machine interfaces, brain signal analysis, and applied machine learning for health.
\end{IEEEbiography}
