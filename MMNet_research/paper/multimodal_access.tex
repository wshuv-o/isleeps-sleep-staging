...
$2.76$\,M trainable parameters. We benchmark ten deep architectures retrained on this
...
here, each recovering its published Sleep-EDF accuracy to within $0.03$, which indicates
...
\item \label{c:bench}We benchmark the model against thirteen baseline configurations on identical
...
accuracy to within $0.03$, every retrained baseline lands between $0.613$ and $0.716$
...
accuracies opposite are not computed on an identical set. $\approx$ marks an accuracy inferred from the macro-F1 its paper
reports.}
\label{tab:reimpl}
\centering
...
\midrule
\multirow{10}{*}{\shortstack[l]{\emph{Deep networks,}\\\emph{retrained on iSLEEPS}}}
