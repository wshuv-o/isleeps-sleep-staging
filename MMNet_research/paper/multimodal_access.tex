...
$\mmAcc$ accuracy (Cohen's $\kappa$ $\mmK$) and flags the $30$-second epochs that carry a
respiratory event at $\mmAuc$ AUC, an epoch-level read-out and not an event-level one,
...
between $0.613$ and $0.720$ accuracy. Six are reimplemented here, and the five that publish
...
%=====================================================================
\begin{table*}[t]
\caption{Recent single-channel sleep-staging methods: the headline figure each reports on
healthy or
general cohorts, set against its accuracy on the subacute-stroke cohort (iSLEEPS) for the five
architectures we evaluated on it. Every reported figure is an accuracy except U-Time's, which
its paper gives as macro-F1 and is labelled accordingly; the two are not interchangeable and
that row should not be read as a like-for-like difference. A dash marks a value we did not evaluate.}
\label{tab:litreview}
\centering
\small
...
U-Time~\cite{perslev2019utime} & 2019 & Fully convolutional encoder--decoder segmenting a whole night at once, no recurrence over epochs. & $0.79$ macro-F1 (Sleep-EDF) & $0.646$ \\
