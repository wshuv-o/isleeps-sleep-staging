...
corpus. The corpus paper reports $0.747$ for its own LSTM, above our $\mmAcc$, on an
$80/10/10$ split over $N=100$ that includes the duplicated pair we exclude. We do not carry
that figure into Table~\ref{tab:bench}, because it was not measured on the folds every other
row there uses; instead we reimplemented that architecture and ran it on our folds, where it
reaches $0.720 \pm 0.032$ over thirty fold-values, a shortfall of $0.027$ that sits inside
the range our other reimplementations show against their published figures. Both readings are
available to the reader: their number here, ours in the table. No method we are aware of
exceeds $0.75$ on this corpus. Section~\ref{sec:curve} shows why: the
