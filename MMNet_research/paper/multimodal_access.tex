...
\caption{The two-stream, multi-task model. Expert physiological features and a frozen
self-supervised embedding are concatenated into a 388-dimensional neural input; the
cardiorespiratory stream is the raw $7\times750$ tensor read by a learned convolutional
encoder. Both encoders are drawn with their internals expanded, since each is instantiated
once. Layer shapes are given above each block and the operation below it; block height
encodes units for the multilayer perceptron and temporal length for the convolutional
stack, so the trade that stack makes is visible --- time resolution is spent to buy
channels, $750\rightarrow23$ samples against $7\rightarrow96$. The governing equations
are given in the text rather than on the figure. The two embeddings are fused and read across the night by a bidirectional LSTM. A
staging head reads the recurrent state; a respiratory head reads it together with a direct
copy of the cardiorespiratory embedding, so desaturation and effort cues reach the
respiratory decision without passing through the staging-oriented fusion. Cross-modal
fusion is the one block detailed separately (right), because attention does not expand
compactly in line. Dashed outlines mark frozen parameters.}
\label{fig:arch}
\end{figure*}
...
detects respiratory events (Table~\ref{tab:ablate}). }
\label{tab:bench}
\centering
...
column in \textbf{bold}. The neural-only and full rows are both means over the same thirty
fold-values, so the difference between them isolates the cardiorespiratory stream: it improves
N1 significantly ($+0.019$, Wilcoxon $p=0.045$) and moves no other stage
($p \geq 0.15$).}
\label{tab:perclass}
\centering
...
model, both averaged over the same thirty fold-values. The cardiorespiratory stream's
contribution to staging is concentrated in N1, the transitional stage that is hardest to score
and the one most often coincident with a respiratory event; no other stage moves
significantly.}
...
modality is removed in turn. Removing the neural channels lowers staging and leaves respiratory
AUC flat; removing cardiorespiratory channels does the reverse. Within the cardiorespiratory
stream only effort matters individually, yet removing the stream entirely costs far more than
...
three seeds. Each row removes one modality and keeps the rest. Neural ablations move
staging and leave respiratory AUC flat; cardiorespiratory ablations do the reverse.
Within the cardiorespiratory stream only respiratory effort is individually
necessary, yet removing the stream as a whole costs $2.8\times$ the sum of the
individual removals --- the signature of a redundant distributed code. Largest
...
event prevalence, $0.157$. \textit{(b)} The proposed model's AUC by event type, scored against
epochs carrying no respiratory event, over the $96$ patients with event-type annotation --- the
rarer, more severe events are detected best. Best per column in \textbf{bold}.}
\label{tab:resp}
\centering
\small
\begin{tabular}{lcc}
...
confound and provides the only external test of the respiratory head. ISRUC records each
...
\begin{table}[t]
\caption{External validation on two public corpora. One model trained on all $99$ iSLEEPS
patients, evaluated zero-shot with no dataset-specific tuning. In-domain figures are the
ten-fold patient-independent means. Sleep-EDF has no cardiorespiratory channels, so the
respiratory head cannot be scored there and that branch is fed zeros; training with the branch
zeroed as well gives $0.774$, confirming the figure is not an artefact of the train/test
mismatch. Both external corpora use a different montage from iSLEEPS, so these are lower bounds
on transfer rather than clean estimates of it. The two ISRUC rows are carried over from the
feature-only configuration and are shown for continuity; they have not been recomputed for the
model reported here.}
\label{tab:ext}
\centering
\small
