...
\begin{table*}[t]
\caption{Sleep staging on iSLEEPS, patient-independent: the proposed model against deep
networks trained from scratch, transferred from healthy sleep, or applied to the raw
multichannel signal. Cells are mean $\pm$ SD across folds where per-fold values were
retained --- five folds for the baselines, thirty fold-values (ten folds, three seeds) for
the proposed rows. CNN-ResNet18 is quoted from the corpus paper and AttnSleep retained only
aggregates, so neither carries an interval. ``feat'' marks engineered features; best per
