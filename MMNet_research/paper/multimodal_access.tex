...
column in \textbf{bold}. \stale{[The neural-only row is from the previous configuration; the
ablation grid did not retain per-class F1, so it has not been regenerated and is not directly
comparable to the row below it.]}}
\label{tab:perclass}
\centering
...
\textbf{MM-Net (neural + cardio)} & \textbf{0.78} & 0.31 & \textbf{0.80} & \textbf{0.71} & \textbf{0.75} \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{\stale{[STALE -- per-class F1, neural-only comparison not yet regenerated for the final model]} Per-class staging F1 for the neural-only and the neural-plus-cardiorespiratory
