...
a per-epoch respiratory-event label, with a direct cardiorespiratory path carrying oxygen
desaturation to the respiratory decision at full strength. On iSLEEPS, the first public
...
corpora, the same weights transfer without degradation. Code, features and checkpoints are
released.
\end{abstract}
...
oxygen-desaturation cues reach the decision undiminished (Section~\ref{sec:method}).
...
\begin{table}[t]
\caption{Reimplementation check. Each architecture marked $^{\dagger}$ in
Table~\ref{tab:bench} was rebuilt from its paper, so its score on iSLEEPS is only
informative if the same code reproduces the corpus its authors used. Ten-fold
subject-independent cross-validation on Sleep-EDF Expanded, single Fpz-Cz derivation, the
derivation each paper reports. Published accuracies are as reported by the original
authors; U-Time reports macro-F1 more prominently than accuracy, so its figure is the
approximate accuracy implied by the paper and is marked accordingly.}
\label{tab:reimpl}
\centering
...
evaluated. The upper block retrains each architecture \emph{on} this cohort; the middle
block trains on healthy sleep (Sleep-EDF Expanded) and applies the checkpoint cold, with
its healthy-sleep score alongside as the control that separates a cohort effect from a
weak model. Cells are mean $\pm$ SD across folds where per-fold values were retained:
five folds for the earlier retrained baselines, ten for the reimplemented literature
models, and thirty fold-values (ten folds, three seeds) for the proposed rows. CNN-ResNet18 is quoted from the corpus paper and AttnSleep retained only
aggregates, so neither carries an interval; the zero-shot block is a single pass over all
$99$ patients and has no fold structure. The final row cross-validates the proposed
architecture on healthy sleep instead of stroke, ten-fold and subject-independent, as a
control on the cohort argument; Sleep-EDF has no cardiorespiratory channels, so it is the
neural branch alone and the comparison row is MM-Net (neural only). $^{\dagger}$ marks a model reimplemented by us from its paper, not run from
published code; for these the healthy column is the \emph{same} code cross-validated on
Sleep-EDF, which recovers each published accuracy to within $0.03$
(Table~\ref{tab:reimpl}) and so establishes that their scores here reflect the cohort and
not the reimplementation.
``feat'' marks engineered features. Best per column in \textbf{bold}.}
\label{tab:bench}
\centering
...
\midrule
\multirow{9}{*}{\shortstack[l]{\emph{Deep networks,}\\\emph{retrained on iSLEEPS}}}
...
\multirow{2}{*}{\shortstack[l]{\emph{Proposed feature-based}\\\emph{multi-task model}}}
 & MM-Net (neural only)              & 7\,ch feat  & 2.62\,M & --- & $\mathbf{0.741 \pm 0.017}$ & $0.668 \pm 0.020$ & $\mathbf{0.635 \pm 0.022}$ \\
...
\emph{Same model, healthy sleep}
 & MM-Net (neural only)              & 4\,EEG feat & 2.62\,M & $0.856 \pm 0.027$ & --- & $0.793 \pm 0.036$ & $0.800 \pm 0.036$ \\
\bottomrule
\end{tabular*}
\end{table*}
...
visible, not hidden; it is a fifth of one fold standard deviation.
...
information.} The upper block of Table~\ref{tab:repr} replaces the $14$ engineered cardiorespiratory features
...
\begin{table*}[t]
\caption{What each stream should be represented as. Every other hyperparameter is held
fixed within a block, so one thing varies at a time; ten-fold patient-independent, three
seeds, thirty fold-values per cell, mean $\pm$ SD. $\Delta$ and Holm $p$ are on the metric
that block varies (respiratory AUC for the cardiorespiratory block, staging accuracy for
the neural block) against the engineered-feature baseline of that block, Wilcoxon paired
on fold-values and Holm-corrected within the block.}
\label{tab:repr}
\centering
...
\multirow{4}{*}{\emph{Cardiorespiratory}}
 ...
 & learned CNN, $0.155$\,M (proposed)     & $\mathbf{0.782 \pm 0.040}$ & $0.740 \pm 0.019$ & $+0.084$ & $<0.001$ \\
\midrule
\multirow{4}{*}{\emph{Neural}}
 ...
 & features $+$ pretrained (proposed)   & $0.690 \pm 0.037$ & $\mathbf{0.749 \pm 0.018}$ & $+0.008$ & $\mathbf{0.001}$ \\
...
F1 $0.802$, which is not consistent with a recall collapse to $0.554$, so the argument in
