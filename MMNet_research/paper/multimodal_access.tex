...
three seeds, thirty fold-values per row. Each row removes one modality and keeps the
rest. Largest effect per column in \textbf{bold}.}
\label{tab:ablate}
\centering
...
\midrule
none (full model)        & 0.739 & 0.782 \\
EEG (neural)             & \textbf{0.680} & 0.774 \\
EOG (ocular)             & 0.731 & 0.777 \\
EMG (muscle)             & 0.735 & 0.778 \\
SpO$_2$                  & 0.742 & 0.780 \\
respiratory effort       & 0.741 & 0.744 \\
pulse / HRV              & 0.741 & 0.785 \\
ECG                      & 0.737 & 0.777 \\
airflow                  & 0.740 & 0.778 \\
all cardiorespiratory    & 0.741 & \textbf{0.654} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{The cardiorespiratory features are the bottleneck, and most of the gap needs no new
...
structure unextracted. A random linear projection of the raw waveform adds another $0.016$, so
part of what the features discard is recoverable without learning anything. The learned CNN
reaches $0.7816$, and a $341$\,M-parameter time-series transformer with \emph{random} weights
reaches $0.7947$ --- higher than the trained CNN. Only the pretrained transformer does better
still, at $0.8078$.

That a trained encoder is beaten by an untrained one of larger capacity is the reason we make
no architectural claim for the convolutional encoder. On $99$ patients the useful thing about
these representations is their richness, not that they were fitted to this cohort; the
model reported here uses the CNN because it is two orders of magnitude smaller than the
transformer at $0.026$ AUC less, not because its design is a contribution.
...
cardiorespiratory stream held at the engineered features. Used alone, neither pretrained
encoder beats the $188$ features: LaBraM is significantly worse ($-0.028$, Holm $p<0.001$) and
CBraMod is indistinguishable ($p=0.43$). Concatenated with the features, LaBraM adds
$+0.008$ accuracy (Holm $p=0.001$). Adding a \emph{second} pretrained encoder on top of the
first adds nothing ($-0.004$, $p=0.24$), and neither does CBraMod in place of LaBraM
($+0.004$, Holm $p=0.17$).
...
but the complementarity saturates after one pretrained encoder, so this is not a general
argument for stacking foundation models.

\begin{table}[t]
\caption{Cardiorespiratory representation, with the neural stream fixed. Ten-fold
patient-independent, three seeds, thirty fold-values per row. $\Delta$ and $p$ are against
the $14$ engineered features, Wilcoxon paired on fold-values, Holm-corrected.}
\label{tab:cardiorep}
\centering
\small
\begin{tabular}{lccc}
\toprule
Cardiorespiratory representation & Resp.\ AUC\,$\uparrow$ & $\Delta$ & Holm $p$ \\
\midrule
$14$ engineered features              & 0.698 & ---      & ---      \\
random nonlinear expansion of the $14$ & 0.715 & $+0.018$ & $<0.001$ \\
random linear projection of raw signal & 0.731 & $+0.034$ & $<0.001$ \\
learned CNN, $0.155$\,M (proposed)     & \textbf{0.782} & $+0.084$ & $<0.001$ \\
untrained transformer, $341$\,M        & 0.795 & $+0.097$ & $<0.001$ \\
pretrained transformer, $341$\,M       & 0.808 & $+0.110$ & $<0.001$ \\
\bottomrule
\end{tabular}
\end{table}
...
engineered features. Ten-fold patient-independent, three seeds. $\Delta$ and $p$ are against
the $188$ engineered features, Holm-corrected within the family.}
\label{tab:eegrep}
\centering
\small
\begin{tabular}{lccc}
\toprule
Neural representation & Staging acc\,$\uparrow$ & $\Delta$ & Holm $p$ \\
\midrule
$188$ engineered features            & 0.741 & ---      & ---     \\
LaBraM alone                         & 0.712 & $-0.028$ & $<0.001$ \\
CBraMod alone                        & 0.736 & $-0.004$ & $0.43$  \\
features $+$ LaBraM (proposed)       & \textbf{0.749} & $+0.008$ & $\mathbf{0.001}$ \\
features $+$ CBraMod                 & 0.745 & $+0.004$ & $0.17$  \\
features $+$ CBraMod $+$ LaBraM      & 0.745 & $+0.004$ & $0.23$  \\
\bottomrule
\end{tabular}
\end{table}
