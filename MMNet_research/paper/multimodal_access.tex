...
\end{equation}
This direct connection matters: without it the respiratory decision must survive a
fusion trained mostly for staging, and the mechanical signature of an obstructed breath is
weakened along the way. Section~\ref{sec:ablate} shows which channel that signature lives
in, and it is respiratory effort rather than the oxygen fall that names the event. The trained model holds $2.76$ million parameters. The pretrained encoder that
...
Table~\ref{tab:bench}: ten-fold subject-independent cross-validation on Sleep-EDF
Expanded, single Fpz-Cz derivation. $\approx$ marks a published accuracy inferred from
the macro-F1 its paper reports.}
\label{tab:reimpl}
\centering
...
\emph{Same model, healthy sleep}
 & MM-Net (neural only)              & 4\,EEG feat & 2.62\,M & $0.856 \pm 0.027$ & --- & --- & --- \\
\bottomrule
\end{tabular*}
\end{table*}
...
Detectors trained on the $14$ cardiorespiratory features on the same folds; chance-level
average precision equals the event prevalence, $0.157$. \textit{(b)} AUC by event type
...
of each block, Wilcoxon paired on fold-values. ``adopted'' marks the choice carried into
the final model; each block holds the other stream at its baseline, so neither row is the
full model.}
\label{tab:repr}
\centering
...
much lower event prevalence ($2.4\%$ against $10.1\%$) rather than anything about the
model.
...
respiratory head is not scored there. ISRUC records each subject twice and the
two nights differ sharply in event prevalence, so they are reported separately.}
\label{tab:ext}
\centering
\small
...
it as falsified rather than removing it. Two cautions follow for every other per-class
...
architectures we evaluated on it, each validated against its published Sleep-EDF
accuracy in Table~\ref{tab:reimpl}. A dash marks a value we did not evaluate.}
\label{tab:litreview}
\centering
\small
