...
$2.76$\,M trainable parameters. We benchmark thirteen deep architectures retrained on this
...
\item \label{c:bench}We benchmark the model against sixteen baseline configurations on identical
...
(range $0$ to $60.5\%$); $79$ of $99$ patients above $5\%$.
\end{table}
...
fraction of epochs that carry a scored respiratory event, over the $99$ patients with
respiratory labels. The median, marked by the dashed line, sits at $13\%$, and many
exceed $30\%$, so the cardiorespiratory signal is abundant.}
\label{fig:prev}
\end{figure}
...
paper~\cite{maiti2026isleeps} rather than run here: that row uses that paper's
...
\midrule
\multirow{13}{*}{\shortstack[l]{\emph{Deep networks,}\\\emph{retrained on iSLEEPS}}}
...
corpus paper rather than re-run here, so its protocol is that paper's and not ours and it
...
\caption{Modality-ablation grid: the drop in staging accuracy and in respiratory AUC when
each modality is removed and the model retrained. The two tasks separate cleanly. Staging
falls only for the neural channels and respiratory detection only for effort and the
cardiorespiratory stream as a whole, with everything between flat. Intervals are in
Table~\ref{tab:ablate}.}
...
folds. \textbf{(a)} Event-level operating characteristic: consecutive flagged epochs are
grouped into events per patient, and each point is a decision threshold, labelled with the
fraction of all epochs it flags. Marker area carries that fraction, because sensitivity and
false-alarm rate alone would make the low thresholds look strictly better. The curve turns
back on itself: flagged runs merge, so fewer distinct false alarms are counted even as more
of the night is flagged. \textbf{(b)} Precision--recall; the dashed line is the $0.157$
event prevalence, the no-skill floor.}
\label{fig:rocpr}
\end{figure}
...
The ablation grid (Fig.~\ref{fig:ablationfig}) establishes what the model \emph{needs}:
remove a modality, retrain, and
