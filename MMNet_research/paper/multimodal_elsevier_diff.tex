...
%DIF ADD multimodal_elsevier.tex   Thu Aug 27 23:02:38 2026
...
%=====================================================================
%DIF 6-8c7-16
%DIF < \documentclass[journal]{IEEEtran}
...
\providecommand{\IEEEPARstart}[2]{#1#2}
 %DIF > 
\providecommand{\IEEEauthorrefmark}[1]{}
 %DIF > 
...
\DIFdelbegin %DIFDELCMD < \title{Multimodal Polysomnography for Joint Sleep Staging and Respiratory-Event Detection in Subacute Ischemic Stroke}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{frontmatter}

\DIFaddend 

\DIFdelbegin %DIFDELCMD < \author{Md Wahiduzzaman Sua, Esm-e Moula Chowdhury Abha, and Md Imtiaj Alam Sajin%
%DIFDELCMD < \thanks{The authors are with the Department of Computer Science and Engineering,
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \title{\DIFadd{A Physiologically Interpretable Multimodal Model for Joint Sleep Staging
and Respiratory-Event Detection in Subacute Ischemic Stroke}}

\DIFaddend 

\DIFaddbegin \author[aiub]{\DIFadd{Md Wahiduzzaman Suva}}
\ead{26-94088-2@student.aiub.edu}
\author[aiub]{\DIFadd{Esm-e Moula Chowdhury Abha}}
\ead{26-94089-2@student.aiub.edu}
\author[aiub]{\DIFadd{Md Imtiaj Alam Sajin}}
\ead{26-94090-2@student.aiub.edu}
\author[aiub]{\DIFadd{Md Iftekharul Mobin}\corref{cor1}}
\ead{iftekhar.mobin@aiub.edu}
\cortext[cor1]{Corresponding author.}

\affiliation[aiub]{organization={Department of Computer Science and Engineering,
American International University-Bangladesh},
  city={Dhaka},
  country={Bangladesh}}

\DIFaddend \markboth{Manuscript, July 2026}%
...
respiratory events at $\mmAuc$ area under the ROC curve\DIFaddbegin \DIFadd{, a capability none of the staging
models possess}\DIFaddend . The respiratory read-out is grounded in \DIFdelbegin \DIFdel{real pathophysiology}\DIFdelend \DIFaddbegin \DIFadd{physiology}\DIFaddend : when we remove the
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{keyword}
\DIFaddend Polysomnography, multimodal learning, sleep staging, sleep apnea, ischemic stroke,
\DIFdelbegin \DIFdel{cardiorespiratory signals}\DIFdelend \DIFaddbegin \DIFadd{feature fusion}\DIFaddend , multi-task learning\DIFdelbegin \DIFdel{.
}%DIFDELCMD < \end{IEEEkeywords}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{, cardiorespiratory signals.
}\end{keyword}
...
\subsection{\DIFdelbegin \DIFdel{Two clinical read-outs from one model}\DIFdelend \DIFaddbegin \DIFadd{Implementation and hyperparameters}\DIFaddend }
The model \DIFdelbegin \DIFdel{stages sleep at $\mmAcc$ accuracy, $\mmMf$ macro-F1, }\DIFdelend \DIFaddbegin \DIFadd{is implemented in PyTorch and trained on a single NVIDIA RTX~2060 GPU (6\,GB) under
...
\subsection{\DIFadd{Compared Methods}}
\DIFadd{We compare the model against deep networks evaluated on the same folds: convolutional and
...
}\DIFaddend and \DIFdelbegin \DIFdel{$\mmK$ $\kappa$, and detects
respiratory events at $\mmAuc$ AUC with $\mmAp$ average precision (Table~\ref{tab:main}). One
compactmodel delivers both outputs on unseen patients from a single pass over the recording. Staging at this level on a stroke cohort is }\DIFdelend \DIFaddbegin \DIFadd{$0.69$ accuracy, $15$ to $20$ points below the same architectures' published accuracy on
...
cohort~\cite{bose2026gap}: with under a hundred patients }\DIFaddend a \DIFdelbegin \DIFdel{strong result: }\DIFdelend \DIFaddbegin \DIFadd{convolutional network cannot
...
night's cycles, and }\DIFaddend the \DIFdelbegin \DIFdel{injured brain distorts the very spindles, slow waves, and atonia that scoring depends on, and every epoch is judged on a
patient the model has never seen}\DIFdelend \DIFaddbegin \DIFadd{stage-probability ribbon exposes where the model is confident and where
it hesitates}\DIFaddend .

\DIFdelbegin %DIFDELCMD < \begin{table}[t]
%DIFDELCMD < \caption{The joint multimodal model, ten-fold patient-independent. Staging is reported with
...
%DIFDELCMD < \end{table}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \includegraphics[width=0.86\textwidth]{figures/fig_hypnogram.pdf}
...
\subsection{\DIFdelbegin \DIFdel{Each modality contributes where its physiology lives}\DIFdelend \DIFaddbegin \DIFadd{Per-class Staging Performance}\DIFaddend }
\DIFdelbegin %DIFDELCMD < \label{sec:contrib}
%DIFDELCMD < %%%
\DIFdel{Turning the cardiorespiratory stream on and off, on the
same folds, shows clearly what it buys
(Table~\ref{tab:ablation}, }\DIFdelend \DIFaddbegin \DIFadd{Table~\ref{tab:perclass} breaks staging down by stage. The pattern is the one this cohort
...
stages. The row-normalised confusion matrix (}\DIFaddend Fig.~\DIFdelbegin \DIFdel{\ref{fig:ablation}) }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:confusion}) shows
where the errors land: N1 is absorbed mainly into Wake and N2, and part of N3 is read as N2}\DIFaddend .

\DIFdelbegin \DIFdel{Respiratory-event detection rises from
$\eegAuc$ AUC, the most a model can do from cortical arousals alone, to }\DIFdelend \DIFaddbegin 
...
\subsection{\DIFadd{Modality-Ablation Grid}}
\label{sec:ablate}
...
events at }\DIFaddend $\mmAuc$ \DIFdelbegin \DIFdel{once it reads
oxygen desaturation and breathing effort, and average precision improves from $0.243$ }\DIFdelend \DIFaddbegin \DIFadd{AUC ($\mmAp$ average precision). To test the paper's central claim, that each
...
respiratory AUC from $\mmAuc$ }\DIFaddend to \DIFdelbegin \DIFdel{$\mmAp$. Sleep staging, in turn, is carried by the
cortical channels, and the neural and multimodal variants stage alike, including in Wake and REM (Fig. ~\ref{fig:perclass}) . This is
the sensible division of labor: the brain channels tell the model what stage the patient is in, }\DIFdelend \DIFaddbegin \DIFadd{$0.681$ and leaves staging untouched, }\DIFaddend and \DIFdelbegin \DIFdel{the breathing channels tell it when the patient stops breathing. The attention fusion and a
plain concatenation give the same numbers}\DIFdelend \DIFaddbegin \DIFadd{removing the whole
...
respiratory AUC significantly ($0.711$ vs $\eegAuc$}\DIFaddend , \DIFaddbegin \DIFadd{$p=0.004$) but not staging ($p=0.91$). A
...
redundant for detection. Attention and plain concatenation stage and detect alike, }\DIFaddend so the gain
\DIFdelbegin \DIFdel{is the modality, cleanly attributable}\DIFdelend \DIFaddbegin \DIFadd{comes from the added modality, and the fusion mechanism has little effect}\DIFaddend .

\DIFdelbegin %DIFDELCMD < \begin{table}[t]
%DIFDELCMD < \caption{Effect of the cardiorespiratory stream (ten-fold, same folds). Staging with Viterbi
%DIFDELCMD < smoothing.}
...
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.94\columnwidth]{figures/fig_mm_ablation.pdf}
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.98\columnwidth]{figures/fig_ablation.pdf}
...
\label{tab:ablate}
\DIFaddendFL \centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.82\columnwidth]{figures/fig_mm_perclass.pdf}
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \setlength{\tabcolsep}{8pt}
\begin{tabular}{lcc}
\toprule
...
\subsection{\DIFdelbegin \DIFdel{The respiratory read-out reads physiology, not }\DIFdelend \DIFaddbegin \DIFadd{Physiological Validity of }\DIFaddend the \DIFdelbegin \DIFdel{label}\DIFdelend \DIFaddbegin \DIFadd{Respiratory Read-out}\DIFaddend }
\label{sec:noflow}
...
%=====================================================================
\DIFaddbegin \section*{\DIFadd{Data and Code Availability}}
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibliographystyle{elsarticle-num}
