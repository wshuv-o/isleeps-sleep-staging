...
%DIF ADD multimodal.tex   Sat Sep  5 15:03:00 2026
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.86\columnwidth]{figures/fig_mm_perclass.pdf}
...
\subsection{\DIFadd{External Validation on Two Independent Corpora}}
\label{sec:external}
...
\DIFadd{The two corpora test different things. \textbf{Sleep-EDF Expanded}~\cite{kemp2000sleepedf}
...
\DIFadd{\textbf{The respiratory read-out transfers.} On ISRUC night one the model reaches $0.721$
...
\DIFadd{\textbf{Staging moves in opposite directions on the two corpora, for reasons the data
...
\DIFadd{Comparing the two also isolates the montage effect. On Sleep-EDF, N3 recall collapses to
...
\DIFadd{Two limits keep this in proportion. Accuracy across cohorts with different class balance is
