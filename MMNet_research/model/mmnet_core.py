...
class MMFeatureNet(nn.Module):
    def __init__(self, n_eeg=188, n_card=14, d=128, d_card=64, hidden=128, layers=2,
                 n_cls=5, drop=0.3, fusion="concat", bypass=True, temporal="lstm"):
        super().__init__()
        self.fusion = fusion
        self.bypass = bypass          # direct cardio -> respiratory head
        self.temporal = temporal      # 'lstm' or 'none' (per-epoch only)
        ...
        else:   # same output width, no cross-epoch context
            self.lstm = None
            self.per_epoch = nn.Sequential(nn.Linear(d, 2 * hidden), nn.GELU(),
                                           nn.Dropout(drop))
    ...
    def forward(self, feeg, fcard):
        ...
        if not self.uses_cardio or not self.bypass:
...
def train_fold(tr, va, fusion, eeg_drop, card_drop, epochs=45, patience=8, bs=32, seed=42,
               bypass=True, temporal="lstm", task="both"):
    torch.manual_seed(seed); np.random.seed(seed)
    Fe,Fc,Y,A,Mk = windows(tr, L//2, eeg_drop, card_drop); N = Fe.shape[0]
    model = MMFeatureNet(fusion=fusion, bypass=bypass, temporal=temporal).to(DEV)
    ...
    for ep in range(epochs):
        ...
        for i in range(0, N-bs+1, bs):
            ...
            loss = ls if task == "stage" else la if task == "apnea" else ls + la
